...
export const incrementStreakCount = async (userId, channel, message, ts) => {
  ...
  Object.keys(emojiKeywords).forEach(async (keyword) => {
    if (message.toLowerCase().search(new RegExp('\\b' + keyword + '\\b', 'gi')) !== -1) {
      await react('add', channel, ts, emojiKeywords[keyword])
    }
  })
  await reply(channel, ts, shouldUpdate ? replyMessage : t('messages.streak.nostreak', { scrapbookLink }))
  if (randomWebringPost) {
    await reply(channel, ts, t('messages.webring.random', { randomWebringPost }), true)
  }
}
