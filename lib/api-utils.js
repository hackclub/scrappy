...
export const emojiExists = async (emoji, updateId) => (
  reactionsTable.read({
    filterByFormula: `AND({Emoji Name} = '${emoji}', {Update ID} = '${updateId}')`
  }).then(r => r.length > 0)
)

export const updateExists = async (updateId) => (
  reactionsTable.read({ filterByFormula: `{Update ID} = '${updateId}'` }).then(r => r.length > 0)
)
...
export const getRandomWebringPost = async (user) => {
  ...
  return `https://slack.com/archives/C01504DCLVD/p${messageTs}`
}
...
export const incrementStreakCount = async (userId, channel, ts) => {
  ...
  const reactingEmojis = 'rocket clap fire party-dinosaur sparkles parrot yay exploding_head sauropod tada zap'.split(' ')
  ...
  if (shouldUpdate) {
    ...
    await runSequentially([
      ...
      randomWebringPost ? reply(channel, ts, t('messages.webring.random', { randomWebringPost }), true) : console.log(userId + ' does not have a webring. skipping...')
    ])
    if (userRecord.fields['New Member'] && updatedStreakCount === 1) {
...
export const startupMessage = async () => {
  const latestCommitMsg = await fetch('https://api.github.com/repos/hackclub/scrappy/commits/main').then(r => r.json()).then(d => d[0].commit.message)
