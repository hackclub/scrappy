...
export const forgetUser = async (user) => {
  // get the user's info
  const user = await getUserRecord(user)
  ...
  await fetch(`https://scrapbook.hackclub.com/${user.fields['Username']}`)
}

export const displayStreaks = async (userId, streakCount) => {
  const userRecord = await getUserRecord(userId)

  if (userRecord.fields['Display Streak']) {
    const statusText = slackUser.profile.status_text
    const statusEmoji = `ghost-${streakCount > 7 ? '7+' : streakCount}`
    setStatus(userId, statusText, statusEmoji)
  } else {
    setStatus(userId, '', '')
  }
  // toggle the user's streak visibility in Airtable
  accountsTable.updateWhere(`RECORD_ID() = "${userRecord.id}"`, {
    'Display Streak': !UserRecord.fields['Display Streak']
  })
}
