...
export const getPublicFileUrl = async (urlPrivate, channel, user) => {
  ...
  if (blob.type === 'images/heic') {
    await fetch(file)
      .then(res => res.blob())
      .then(newBlob => heic2any({ newBlob }))
      .then(heic2PngBlob => blob = heic2PngBlob)
      .catch(err => {
        console.log('error converting heic to png', err)
        return { url: 'heic' }
      })
  }
...
export const getMessage = async (ts, channel) => {
  try {
    ...
    if (resp.messages.length < 1) return null
