...
export const getRandomWebringPost = async (user) => {
  ...
  const latestUpdate = await updatesTable.read({
    ...
    filterByFormula: `{Poster ID} = '${randomUserRecord.fields['ID']}'`
  })
...
export const incrementStreakCount = async (userId, channel, ts) => {
  ...
  if (shouldUpdate) {
    ...
    await Promise.all([
      ...
      randomWebringPost ? reply(channel, ts, t('messages.webring.random', { randomWebringPost })) : console.log(userId + ' does not have a webring. skipping...')
    ])
    if (userRecord.fields['New Member'] && updatedStreakCount === 1) {
...
export const reply = async (channel, parentTs, text) =>
