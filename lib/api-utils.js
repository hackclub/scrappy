...
export const getEmojiRecord = async (emoji) => {
  const emoji = await emojiTypeTable.read({
    filterByFormula: `Name = '${emoji}'`,
    maxRecords: 1
  })
  if (emoji.length > 0) return emoji[0]
  else {
    let emojiUrl
    let unicodeEmoji = emoji.find(emoji)
    if (!unicodeEmoji) {
      const emojiList = await fetch(`https://slack.com/api/emoji.list?token=${process.env.SLACK_USER_TOKEN}`).then(r => r.json())
      emojiUrl = emojiList.emoji[emoji]
    }
    return await emojiTypeTable.create({
      'Name': emoji,
      'Image URL': emojiUrl
    })
  }
...
export const incrementStreakCount = async (userId, channel, ts) => {
  ...
  if (shouldUpdate) {
    ...
    await accountsTable.update(userRecord.id, {
      ...
      'Max Streak': updatedMaxStreakCount
    })
