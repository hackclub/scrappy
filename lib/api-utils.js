...
export const formatText = (text) => {
  ...
  if (users) {
    users.forEach(async (user) => {
      ...
      const displayName = userProfile.profile.display_name || userProfile.profile.real_name
      console.log(displayName)
      console.log(text.replace(user, `@${displayName}`))
      text = text.replace(user, `@${displayName}`)
    })
  }
  ...
  if (channels) {
    channels.forEach(async (channel) => {
      ...
      const channelInfo = await fetch(
        `https://slack.com/api/conversations.info?token=${process.env.SLACK_BOT_TOKEN}&channel=${channelId}`
      ).then(r => r.json())
