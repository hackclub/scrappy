...
export const getPublicFileUrl = async (urlPrivate) => {
  ...
  else if (fileName.toLowerCase().includes('heic')) return 'heic'
...
export const incrementStreakCount = async (userId, channel, ts) => {
  ...
  const reactingEmojis = ['summer-of-making', 'rocket','clap', 'fire', 'party-dinosaur', 'parrot','yay','exploding_head','sauropod','tada','zany_face','zap' ]
  const randomEmoji = reactingEmojis[Math.floor(Math.random() * reactingEmojis.length)];
