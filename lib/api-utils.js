...
export const getUserRecord = async (userId) => {
  ...
  if (typeof record === 'undefined') {
    ...
    console.log(`No user record found for ${userId}. Creating...`, username, tzOffset, tz)
    ...
    if (!user.profile.is_custom_image) {
      const animalImages = [
        'https://i.imgur.com/njP1JWx.jpg', 'https://i.imgur.com/NdOZWDB.jpg', 'https://i.imgur.com/l8dV3DJ.jpg',
        'https://i.imgur.com/Ej6Ovlq.jpg', 'https://i.imgur.com/VG29lvI.jpg', 'https://i.imgur.com/tDusvvD.jpg',
        'https://i.imgur.com/63H1hQM.jpg', 'https://i.imgur.com/xGtLTa3.png'
      ]
      const animalImage = sample(animalImages)
      console.log(`User ${userId} doesn't have a profile picture set. Setting to ${animalImage}`)
...
export const emojiExists = async (emoji, updateId) => (
  ...
  }).then(r => r.length > 0).catch(err => console.log('Cannot check if emoji exists', err))
)

export const updateExists = async (updateId) => (
  reactionsTable.read({ filterByFormula: `{Update} = '${updateId}'` }).then(r => r.length > 0).catch(err => console.log('Cannot check if update exists', err))
)
...
export const getEmojiRecord = async (reaction) => {
  ...
  if (emojiRecord.length > 0) return emojiRecord[0]
    ...
    if (!unicodeEmoji) {
      ...
      const emojiList = await fetch(`https://slack.com/api/emoji.list?token=${process.env.SLACK_USER_TOKEN}`).then(r => r.json())
...
export const getRandomWebringPost = async (user) => {
  ...
  if (!webring) { console.log('no webring found'); return; }
  console.log('webring for user exists! yay!')

  const randomWebringId = sample(webring)
  console.log('random webring id', randomWebringId)
  const randomUserRecord = await accountsTable.read({ filterByFormula: `{Record ID} = '${randomWebringId}'` })
...
export const isFullMember = async (userId) => {
  const user = await fetch(`https://slack.com/api/users.info?token=${process.env.SLACK_BOT_TOKEN}&user=${userId}`).then(r => r.json())
...
export const getReplyMessage = async (user, username, day) => {
  ...
  if (!newMember && day <= 3) {
    ...
    return t('messages.streak.oldmember.' + streakNumber, { scrapbookLink, user })
  }
...
export const incrementStreakCount = async (userId, channel, message, ts) => {
  ...
  if (shouldUpdate) {
    ...
    const emojiArray = [["yay","yay"],
                        ...
                        ["drive","car"],
                        ["bank","bank-hackclub"],
                        ["shopping list","hardware-2"]]

    emojiArray.forEach(function (item, index) {
      if (message.includes(item[0])){
        await react('add', channel, ts, item[1])
      }
    });

    await reply(channel, ts, replyMessage)
    randomWebringPost ? await reply(channel, ts, t('messages.webring.random', { randomWebringPost }), true) : console.log(userId + ' does not have a webring. skipping...')
    ...
    await react('remove', channel, ts, 'beachball')
    await react('add', channel, ts, 'summer-of-making')
    await reply(channel, ts, t('messages.streak.nostreak', { scrapbookLink }))
    randomWebringPost ? await reply(channel, ts, t('messages.webring.random', { randomWebringPost }), true) : console.log(userId + ' does not have a webring. skipping...')
  }
}
