...
export const getReplyMessage = (userId, username, day, newMember) => {
  const scrapbookLink = `https://scrapbook.hackclub.com/${username}`
  const messages = {
    1: newMember ?
      `Groovy! Congratulations <@${userId}> on your first post in <#C015BFLDG93>. Your site has been started at ${scrapbookLink}. 
    \n\nIf you want to display your current streak count, type \`/summerstreaks\`. If you want to add custom CSS to your profile,
    type \`/setcss <link to css file or GitHub Gist>\`.
    \n\nMake sure to post every day, or your streak will reset!` :
      `Welcome back to your first post, <@${userId}>! ${scrapbookLink}`,
    ...
    7: `A WHOLE WEEK OF STREAKS, you are on fire :fire: ${scrapbookLink}`,
    '7+': `https://www.youtube.com/watch?v=i0dmSIAzWeQ \n \n ${scrapbookLink} \n \n <${`https://twitter.com/intent/tweet?text=This%20summer%20I%20am%20making%20loads%20of%20things.%20Track%20my%20progress%20at%20${scrapbookLink}`}|Why not share your amazing work on Twitter? >`
  }
  if (day <= 7) {
    return messages[day]
  } else {
