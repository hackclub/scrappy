...
*/
const { react, postEphemeral, updatesTable, getUserRecord, formatInHTMLWithEmoji, fetchProfile, unverifiedRequest } = require("../../../../lib/api-utils")

export default async (req, res) => {
  if (unverifiedRequest(req)) return res.status(400).send('Unverified Slack request!')
  else res.status(200).end()

  const newMessage = formatInHTMLWithEmoji(req.body.event.message.text)
