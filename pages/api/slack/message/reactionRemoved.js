...
export default async (req, res) => {
  ...
  limiter.schedule(async () => {
    ...
    if (updatedUsersReacted.length === 0) {
      console.log(startTS, 'i deleted the reaction record because that was the only reaction!')
    ...
    console.log(startTS, 'Finished removing reaction, took', Date.now() - startTS, 'ms')
  })
}