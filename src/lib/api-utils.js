...
export const getEmojiRecord = async (reaction) => {
  ...
  if (emojiRecord.length > 0) return emojiRecord[0];
    ...
    if (!unicodeEmoji) {
      ...
      const emojiList = await fetch(`https://slack.com/api/emoji.list`, {
        headers: {
          Authorization: `Bearer ${process.env.SLACK_BOT_TOKEN}`,
        },
      }).then((r) => r.json());
...
export const isFullMember = async (userId) => {
  const user = await fetch(`https://slack.com/api/users.info?user=${userId}`, {
    headers: {
      Authorization: `Bearer ${process.env.SLACK_BOT_TOKEN}`,
    },
  }).then((r) => r.json());
  ...
  if (users) {
    await Promise.all(
      users.map(async (u) => {
        ...
        if (!userRecord) {
          fetch(
            `https://slack.com/api/users.profile.get?user=${u.substring(
              2,
              u.length - 1
            )}`,
            {
