...
(async () => {
  ...
  const createMany = await prisma.accounts.createMany({
    data: read.map((x) => ({
      ...
      displayStreak: x.fields["Display Streak"]
        ? x.fields["Display Streak"]
        : true,
      streaksToggledOff: x.fields["Streaks Toggled Off"]
        ? x.fields["Streaks Toggled Off"]
        : false,
      customDomain: x.fields["Custom Domain"]
        ? x.fields["Custom Domain"]
        : null,
      ...
      fullSlackMember: x.fields["Full Slack Member?"]
        ? x.fields["Full Slack Member?"]
        : false,
      avatar: x.fields["Avatar"]
        ? x.fields["Avatar"][0].url
        : "https://cloud-pwcbafyg3-hack-club-bot.vercel.app/0mel.png",
      webring: x.fields["Webring Slack Usernames"]
        ? x.fields["Webring Slack Usernames"]
        : [],
      newMember: x.fields["New Member"] ? x.fields["New Member"] : true,
      timezoneOffset: x.fields["Timezone offset"]
        ? x.fields["Timezone offset"]
        : 0,
